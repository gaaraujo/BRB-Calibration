...
| Calibrate + plot one specimen | Edit `scripts/calibrate_single/input.csv` (one or more `meta.set_id` values; per-set comma-separated fields), then `scripts/calibrate_single/calibrate_one_specimen.py <Name>`; optional `--prepare-data` |
